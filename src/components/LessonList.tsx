...
export default function LessonList({ lessons, onStartLesson }: LessonListProps) {
  return (
    <div className="glass overflow-hidden">
      {lessons.map((lesson, i) => (
        <button
          ...
          className={`flex items-center gap-4 px-5 py-4 transition-colors w-full text-left ${
            lesson.completed
              ? "bg-blue-50/50 cursor-default"
              : "hover:bg-gray-50 cursor-pointer active:bg-gray-100"
          } ${i < lessons.length - 1 ? "border-b border-gray-100" : ""}`}
        >
