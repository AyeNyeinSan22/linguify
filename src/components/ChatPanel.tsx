...
export default function ChatPanel({ messages, isLoading, streamingText }: {
  messages: Message[]; isLoading: boolean; streamingText?: string | null;
}) {
    ...
    <div className="glass-heavy flex flex-col gap-4 p-5 min-h-[420px] max-h-[540px] overflow-y-auto bg-[#FAFAFA]">
      {messages.length === 0 && !streamingText && (
        <div className="flex flex-1 items-center justify-center">
          <p className="text-sm text-text-muted">Send a message to start your session.</p>
        </div>
      )}
      {messages.map((msg, i) => (
        <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
          <div className={`max-w-[80%] px-4 py-3 text-sm leading-relaxed ${msg.role === "user" ? "chat-bubble-user" : "chat-bubble-coach"}`}>
            {msg.meta && <span className="mb-1.5 block text-[11px] font-medium opacity-60">{msg.meta}</span>}
      ...
      {streamingText && (
        <div className="flex justify-start">
          <div className="chat-bubble-coach max-w-[80%] px-4 py-3 text-sm leading-relaxed">
            <p className="whitespace-pre-wrap">{streamingText}<span className="streaming-cursor" /></p>
          </div>
        </div>
      )}
