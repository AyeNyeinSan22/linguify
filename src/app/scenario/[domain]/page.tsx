...
> = {
  restaurant: {
    ...
    accentColor: "text-[#FF7043]",
    accentBg: "bg-[#FF7043]",
    accentBorder: "border-[#FF7043]",
    color: "text-[#FF7043]",
    bg: "bg-[#FF7043]/8",
  ...
  hotel: {
    ...
    accentColor: "text-[#26A69A]",
    accentBg: "bg-[#26A69A]",
    accentBorder: "border-[#26A69A]",
    color: "text-[#26A69A]",
    bg: "bg-[#26A69A]/8",
  ...
  train: {
    ...
    accentColor: "text-[#1E88E5]",
    accentBg: "bg-[#1E88E5]",
    accentBorder: "border-[#1E88E5]",
    color: "text-[#1E88E5]",
    bg: "bg-[#1E88E5]/8",
  ...
  attraction: {
    ...
    accentColor: "text-[#8E24AA]",
    accentBg: "bg-[#8E24AA]",
    accentBorder: "border-[#8E24AA]",
    color: "text-[#8E24AA]",
    bg: "bg-[#8E24AA]/8",
...
export default function ScenarioDetailPage({
  ...
  const [lessonFocus, setLessonFocus] = useState<string | undefined>(
    undefined
  );

  useEffect(() => {
    fetch(`/api/scenarios?domain=${domain}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.domain) {
          setDetail(d.domain);
    ...
    fetch("/api/scenarios")
      ...
      .then((d) => {
        if (d.domains)
          setRelated(
            d.domains.filter((x: DomainDetail) => x.name !== domain).slice(0, 3)
          );
      })
      .catch(() => {});
  }, [domain]);
  ...
  const startPractice = async (lessonTitle?: string) => {
    ...
    setLessonFocus(lessonTitle);
    try {
      const r = await fetch("/api/scenarios", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ domain, type: "roleplay" }),
      });
      const d = await r.json();
      if (d.scenario) {
        setActiveScenario({
          context: d.scenario.context,
          prompt: d.scenario.prompt + topicHint,
        ...
        setMessages([
          {
            role: "coach",
            content: `Couldn't load scenario: ${d.error || "Unknown error"}`,
          },
        ]);
        setChatStarted(true);
      }
      ...
      setMessages([
        { role: "coach", content: "Could not reach the server. Please try again." },
      ]);
      setChatStarted(true);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full bg-[var(--accent-400)] animate-bounce [animation-delay:0ms]" />
          <span className="h-2 w-2 rounded-full bg-[var(--accent-500)] animate-bounce [animation-delay:150ms]" />
          <span className="h-2 w-2 rounded-full bg-[var(--accent-600)] animate-bounce [animation-delay:300ms]" />
        </div>
      </div>
    );
  if (error)
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <button onClick={() => router.push("/agent")} className="btn-gradient">
          Browse Scenarios
        </button>
      </div>
    );
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8 py-8 sm:py-12 flex flex-col gap-8">
      <ScenarioHeader
        icon={detail.icon}
        title={detail.label}
      ...
      <div className={`grid gap-8 ${chatStarted ? "lg:grid-cols-2" : ""}`}>
        ...
        {chatStarted && (
          <div className="flex flex-col gap-5">
            ...
            {/* Chat */}
            {messages.length > 0 && (
              <ChatPanel messages={messages} isLoading={chatLoading} />
            )}
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Reply as if you're really there…"
      ...
      {!chatStarted && related.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {related.map((d) => {
              const c = DOMAIN_STYLES[d.name] || DOMAIN_STYLES.restaurant;
              return (
                <CourseCard
                  key={d.name}
                  icon={d.icon}
